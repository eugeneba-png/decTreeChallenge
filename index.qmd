...
Answer: Zip code should be modelled categorically, not as a  single numeric feature. It is a non-ordinal variable. The numeric tree + scatter together show that signal is in “which zip,” not in the numeric value of the zip → model as categorical; zip code is non-ordinal. 
...
Answer: How the algorithm “sees” zip code

The key difference is how Python and R handle categorical variables like zip codes in decision trees. In Python’s sklearn.tree.DecisionTreeRegressor, data must be numeric, so zip codes are either treated as ordered integers (which is misleading) or converted into one-hot encoded columns, leading to binary threshold splits on each column and feature importance being spread across many variables. In contrast, R’s typical approach using factors (e.g., with rpart) allows trees to split directly on categories by grouping levels, producing more natural and interpretable rules. As a result, both the tree structure and feature importance differ between the two approaches. While scikit-learn’s trees are still valid, they lack native categorical handling, making R’s factor-based trees more aligned with traditional categorical modeling unless alternative Python methods or libraries are used.
Quote 1 (advantages section):
...
Quote 2 (CART / algorithms section):
...
Those lines are the direct documentation link between “model zip as categorical” and a weakness of the Python (sklearn) tree module: you must represent categories numerically yourself (e.g. dummy variables), which changes splits, the plotted tree, and how importance is distributed across features compared to R workflows that pass factors into tree functions direct
...
Answer: Yes, you can get a decision tree in Python that splits on subsets of zip codes. The resulting tree splits on subsets of zip codes rather than numerical thresholds. The tree shows that the signal is in “which zip,” not in the numeric value of the zip.
